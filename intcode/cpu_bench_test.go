...
func BenchmarkAckermann(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		cpu, out := NewCPUForProgram(ackermannProgram, input.NewFixed(3, 6))
		...
		if result != 509 {
			b.Errorf("Expected 509 but got %d", result)
		}
	}
}
...
func BenchmarkISqrt(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		cpu, out := NewCPUForProgram(isqrtProgram, input.NewFixed(130))
		...
		if result != 11 {
			b.Errorf("Expected 11 but got %d", result)
		}
	}
}
...
func BenchmarkDivMod(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		cpu, out := NewCPUForProgram(divmodProgram, input.NewFixed(1024, 3))
		...
		wg := output.Each(out, func(i int) {
			...
			if i != want {
				b.Errorf("Expected %d but got %d", want, i)
			}
		})
...
func BenchmarkPrimeFactors_2147483647(b *testing.B) {
	b.ReportAllocs()

	for i := 0; i < b.N; i++ {
		cpu, out := NewCPUForProgram(factorProgram, input.NewFixed(2147483647))
		...
		if result != 2147483647 {
			b.Errorf("Expected 2147483647 but got %d", result)
		}
	}
}

func BenchmarkPrimeFactors_19201644899(b *testing.B) {
	b.ReportAllocs()

	for i := 0; i < b.N; i++ {
		cpu, out := NewCPUForProgram(factorProgram, input.NewFixed(19201644899))

		expected := []int{138569, 138571}
		wg := output.Each(out, func(i int) {
			var want int
			want, expected = expected[0], expected[1:]
